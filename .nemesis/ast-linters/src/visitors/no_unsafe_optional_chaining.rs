...
fn check_optional_chain(node: &tree_sitter::Node, source: &str, violations: &mut Vec<Violation>) {
    ...
    match parent.kind() {
        "binary_expression" => {
            ...
            ).with_suggestion("[STOP] Leia .devin/rules/typescript-typing-convention.md antes de reescrever. Verifique se o valor pode ser undefined antes de operar."));
        }
        "member_expression" => {
            ...
            if let Some(gp) = grandparent {
                if gp.kind() == "call_expression" {
                    ...
                    violations.push(Violation::new(
                        ...
                        line, RuleCategory::Correctness,
                    ).with_suggestion("[STOP] Leia .devin/rules/typescript-typing-convention.md antes de reescrever. Use (obj?.method)?.() ou verifique antes de chamar."));
                }
            }
        }
        _ => {}
