...
fn check_call_expression(node: &tree_sitter::Node, source: &str, violations: &mut Vec<Violation>) {
    ...
    if is_promise_returning_function(node, source) {
        ...
        violations.push(
            ...
            .with_suggestion("[STOP] Leia .windsurf/rules/react-hooks-patterns-rules.md antes de reescrever. Use await, .then().catch() ou void se intencional. Consulte: https://biomejs.dev/")
        );
    }
}
